...
func TestTransferTiming(t *testing.T) {
	...
	check := func(at time.Time, elapsed *uint64, speed uint64, eta *uint64) {
		...
		failIfFmt(t, !equal(x.ElapsedMS, elapsed) || x.SpeedBPS != speed || !equal(x.ETASeconds, eta), "timing at %v: %+v, elapsed=%v eta=%v", at.Sub(now), x, x.ElapsedMS, x.ETASeconds)
	}
	...
	failIfFmt(t, err != nil || !strings.Contains(string(encoded), `"elapsed_ms":null`) || !strings.Contains(string(encoded), `"eta_seconds":0`), "nullable JSON: %s %v", encoded, err)
}
...
func TestTimingLifecycleGuards(t *testing.T) {
	...
	failIf(t, s.transferTiming[id].known, "upload setup started timing")
	bad := event
	bad.Attempt++
	s.uploadStreamStart(1, bad)
	failIf(t, s.transferTiming[id].known, "stale stream start accepted")
	event.Done = 30
	s.uploadStreamStart(1, event)
	failIf(t, !s.transferTiming[id].known || s.transferTiming[id].sampleDone != 30, "upload stream offset not recorded")
	event.State = "failed"
	s.uploadUpdate(1, event)
	...
	event.State = "queued"
	s.uploadUpdate(1, event)
	failIf(t, s.transferTiming[id].elapsed != prior || !s.transferTiming[id].known, "retry lost timing")
...
func TestTransferRetryCountdownIsReadOnly(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		failIfFmt(t, got.RetryInSeconds == nil || *got.RetryInSeconds != tc.want || got.WaitingForPeerSeconds != nil || got.State != original.State || got.Error != original.Error || got.Done != original.Done, "countdown at %v: %+v", tc.at, got)
		failIf(t, s.journal.Downloads[0] != download || s.transfers[original.ID] != original, "reading countdown changed retry policy/state")
	}
	...
	for _, got := range []Transfer{s.Transfers()[0], s.Snapshot().Transfers[0]} {
		failIfFmt(t, got.RetryInSeconds == nil || *got.RetryInSeconds != 0 || got.State != "retrying" || got.Error != original.Error, "public snapshot: %+v", got)
	}
