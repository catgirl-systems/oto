...
func uploadService(t *testing.T) (*Service, *soulseek.UploadManager, *soulseek.UploadJob, string) {
	...
	for _, name := range []string{"one", "two"} {
		must(t, os.WriteFile(filepath.Join(root, name), []byte("shared"), 0600))
	}
...
func TestUploadActionsLifecycle(t *testing.T) {
	...
	for _, item := range [][2]string{{"alice", `Music\one`}, {"alice", `Music\two`}, {"bob", `Music\one`}} {
		_, err := s.client.QueueUpload(item[0], item[1])
		must(t, err)
	}
	...
	failIfFmt(t, err != nil || result.Changed != 2, "abort users: %+v %v", result, err)
	failIf(t, uploadRow(t, s, "bob", `Music\one`).State != "queued", "unrelated user cancelled")
	failIf(t, uploadRow(t, s, "alice", `Music\one`).State != "cancelled", "missing cancelled history")
	result, err = s.UploadAction(UploadActionRequest{Action: "retry", IDs: []string{one, one}})
	failIfFmt(t, err != nil || result.Changed != 1 || uploadRow(t, s, "alice", `Music\one`).State != "queued", "retry: %+v %v", result, err)
	must(t, s.TransferAction(one, "cancel"))
	result, err = s.UploadAction(UploadActionRequest{Action: "clear", States: []string{"cancelled"}})
	failIfFmt(t, err != nil || result.Changed != 2, "clear cancelled: %+v %v", result, err)
	result, err = s.UploadAction(UploadActionRequest{Action: "clear", All: true})
	failIfFmt(t, err != nil || result.Changed != 1, "clear all: %+v %v", result, err)
	...
	failIf(t, err != nil || string(content) != "shared", "shared file changed")
	...
	failIf(t, uploadRow(t, s, "alice", `Music\one`).State != "queued", "requeue invisible")
}

func TestUploadClearCountsCallbackAutoClear(t *testing.T) {
	s, _, _, _ := uploadService(t)
	s.cfg.Uploads.AutoClearCancelled = true
	_, err := s.client.QueueUpload("peer", `Music\one`)
	must(t, err)
	id := uploadRow(t, s, "peer", `Music\one`).ID
	result, err := s.UploadAction(UploadActionRequest{Action: "clear", IDs: []string{id}})
	failIfFmt(t, err != nil || result.Changed != 1 || result.Skipped != 0 || len(result.Errors) != 0, "clear callback result: %+v %v", result, err)
	failIf(t, len(s.Transfers()) != 0, "callback-cleared upload remains visible")
	...
	failIfFmt(t, err != nil || result.Changed != 1 || result.Skipped != 0 || len(result.Errors) != 0, "cancel callback result: %+v %v", result, err)
}

func TestUploadActionValidationAndStates(t *testing.T) {
	s, _, _, _ := uploadService(t)
	_, err := s.client.QueueUpload("a", `Music\one`)
	must(t, err)
	...
	for _, req := range []UploadActionRequest{
		...
		failIf(t, uploadRow(t, s, "a", `Music\one`).State != "queued", "invalid batch mutated state")
	}
	result, err := s.UploadAction(UploadActionRequest{Action: "cancel", IDs: []string{id, "upload:missing"}})
	failIfFmt(t, err != nil || result.Changed != 1 || len(result.Errors) != 1, "mixed result %+v %v", result, err)
	result, err = s.UploadAction(UploadActionRequest{Action: "cancel", IDs: []string{id}})
	failIfFmt(t, err != nil || result.Skipped != 1, "idempotent abort %+v %v", result, err)
	s.mu.Lock()
	...
	failIfFmt(t, err != nil || result.Changed != 0, "empty match %+v %v", result, err)
}

func TestUploadRetirementAndStaleUpdates(t *testing.T) {
	s, _, _, _ := uploadService(t)
	c := s.client
	_, err := c.QueueUpload("a", `Music\one`)
	must(t, err)
	id := uploadRow(t, s, "a", `Music\one`).ID
	s.mu.RLock()
	...
	failIf(t, len(s.Transfers()) != 0, "stale progress resurrected clear")
	if _, err := c.QueueUpload("a", `Music\one`); err != nil {
		t.Fatal(err)
	}
	s.uploadUpdate(1, stale)
	failIf(t, uploadRow(t, s, "a", `Music\one`).State != "queued", "old attempt replaced new attempt")
	s.lifecycleMu.Lock()
	s.stopSessionLocked(true)
	s.lifecycleMu.Unlock()
	failIf(t, uploadRow(t, s, "a", `Music\one`).State != "interrupted", "offline left a live upload")
	stale.State = "queued"
	stale.Attempt += 100
	s.uploadUpdate(1, stale)
	failIf(t, uploadRow(t, s, "a", `Music\one`).State != "interrupted", "retired session update accepted")
}
