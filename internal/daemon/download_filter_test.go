...
func TestFilteredDownloadsPersistAndRequireExplicitForce(t *testing.T) {
	...
	failIfFmt(t, err != nil || len(rows) != 3 || rows[0].State != "filtered" || rows[1].State != "queued" || rows[2].State != "filtered", "queue: %+v %v", rows, err)
	...
	failIf(t, len(s.journal.Downloads) != 3, "invalid batch partially committed")
	...
	failIfFmt(t, err != nil || result.Changed != 2 || result.Skipped != 1, "force: %+v %v", result, err)
	...
	failIf(t, err != nil || again[0].State != "filtered", "bypass escaped its record")
	files, err := os.ReadDir(cfg.DownloadDir)
	failIf(t, err != nil && !os.IsNotExist(err), err)
	failIfFmt(t, len(files) > 0, "offline/filtered records created files: %v", files)
}
