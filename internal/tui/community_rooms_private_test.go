...
func TestCommunityPrivateRoomsFormsDraftsAndLayouts(t *testing.T) {
	...
	failIf(t, !m.community.rooms.createPrivate, "private creation toggle unavailable")
	...
	failIf(t, !p.wallForm || p.wallInput != "q/?猫😀" || m.help || m.community.chats.composing, "wall input escaped into global/chat keys", p.wallInput)
	updated, cmd := m.Update(tea.PasteMsg{Content: "\r\nline"})
	m = updated.(model)
	drainActivity(t, &m, cmd)
	failIf(t, p.wallInput != "q/?猫😀\nline", "paste sent or failed to preserve text", p.wallInput)
	before := p.wallInput
	m.pasteCommunityPrivate("\x1b[2J")
	failIf(t, p.wallInput != before || p.err == "", "terminal control paste accepted")
	for _, size := range [][2]int{{120, 40}, {80, 24}, {40, 16}, {20, 6}} {
		m.width, m.height = size[0], size[1]
		view := m.View().Content
		failIfFmt(t, lipgloss.Width(view) > m.width || lipgloss.Height(view) > m.height, "wall form overflow %v: %dx%d", size, lipgloss.Width(view), lipgloss.Height(view))
		failIf(t, p.wallInput != before, "resize lost draft")
	}
	m.width, m.height = 120, 40
	m.key(tea.KeyPressMsg(tea.Key{Code: tea.KeyEscape}))
	failIf(t, m.chatDraftCount() != 1 || !m.confirmChatDraftQuit() || m.community.chats.dialog.confirm, "wall draft bypasses Cancel-default quit")
	...
	failIf(t, p.wallDrafts[m.wallDraftKey()].text != before, "reconnect lost draft")
	p.wallReady = true
	m.key(chatPress("i"))
	failIf(t, p.wallInput != before, "reopening wall lost draft")
	m.key(tea.KeyPressMsg(tea.Key{Code: tea.KeyEscape}))
	summary.Account = "other account"
	summary.Session++
	m.applyCommunitySummary(communitySummaryMsg{request: m.community.summaryRequest, summary: summary})
	failIf(t, p.wallForm || p.wallInput != "" || len(p.wallDrafts) != 1, "account switch exposed or destroyed private draft")
}
...
func TestCommunityPrivateRoomsRolesConfirmationAndVisibility(t *testing.T) {
	...
	r.membersFresh = true
	m.key(chatPress("M"))
	failIf(t, p.view != "roles" || m.communityTranscriptSelected() || m.chatTranscriptVisible(), "roles marks hidden history read")
	m.key(chatPress("a"))
	for _, ch := range "q/?" {
		m.key(chatPress(string(ch)))
	}
	failIf(t, p.roleInput != "q/?" || m.help, "role form intercepted literal keys")
	p.roleInput = "Bob"
	m.key(tea.KeyPressMsg(tea.Key{Code: tea.KeyEnter}))
	failIf(t, p.dialog == nil || p.dialog.confirm || !strings.Contains(p.dialog.label, "Bob"), "missing exact-user Cancel-default confirmation")
	...
	m.key(tea.KeyPressMsg(tea.Key{Code: tea.KeyEscape}))
	m.key(chatPress("o"))
	failIf(t, p.dialog == nil || m.statusMenu, "grant operator opened global presence menu")
	p.dialog = nil
	r.active.Role = "operator"
	failIf(t, m.roleActionAllowed(soulseek.RoomAddOperator) || !m.roleActionAllowed(soulseek.RoomAddMember), "operator UI permission matrix")
	r.active.Role = "member"
	failIf(t, m.roleActionAllowed(soulseek.RoomAddMember) || !m.roleActionAllowed(soulseek.RoomCancelMembership), "member UI permission matrix")
	r.active.RoleFresh = false
	failIf(t, m.roleActionAllowed(soulseek.RoomCancelMembership), "stale role authorized action")
	for _, size := range [][2]int{{120, 40}, {80, 24}, {40, 16}, {20, 6}} {
		m.width, m.height = size[0], size[1]
		for _, view := range []string{"roles", "wall"} {
			p.view = view
			screen := m.View().Content
			failIfFmt(t, lipgloss.Width(screen) > m.width || lipgloss.Height(screen) > m.height, "%s overflow %v", view, size)
		}
	}
	...
	m.key(chatPress("W"))
	m.key(chatPress("I"))
	failIf(t, p.view != "" || p.dialog != nil, "private controls available without capability")
}
...
func TestCommunityPrivateRoomsStaleResourcesAndDraftSafeResults(t *testing.T) {
	...
	failIf(t, r.active.Role != "revoked", "older cross-resource response restored revoked authority", r.active)
	...
	failIf(t, p.wallReady || p.wallDrafts[m.wallDraftKey()].text != "local draft", "canceled response modified fresh view/draft")
	p.wallForm = true
	p.wallInput = "a\nb"
	m.privateRoomFormKey(tea.KeyPressMsg(tea.Key{Code: tea.KeyEnter}))
	failIf(t, p.dialog == nil || p.dialog.confirm || p.dialog.text != "a b", "multiline wall did not require flattened preview")
	p.dialog = nil
	p.wallForm = false
	m.privateRoomKey(chatPress("C"))
	failIf(t, p.dialog == nil || p.dialog.confirm, "clear not Cancel-default")
...
func TestCommunityPrivateRoomsReviewRegressions(t *testing.T) {
	...
	failIf(t, r.active.Role != "none" || r.activeRevision != 100, "unversioned action reply restored authority")
	p.view = "roles"
	m.community.pane = 0
	...
	failIf(t, r.row != 1 || r.memberRow != 0, "focused directory navigation changed hidden role selection")
	m.community.pane = 2
	m.key(tea.KeyPressMsg(tea.Key{Code: tea.KeyEnter}))
	failIf(t, m.community.target != "Alice" || p.roleForm != "", "focused roster Enter operated on hidden role pane")
	m.community.target = ""
	m.community.pane = 1
	...
	failIf(t, !strings.Contains(m.privateRoomDialogView(), "TAIL-OF-PREVIEW") || p.dialog.confirm, "cannot inspect complete preview safely")
	m.privateRoomDialogKey(tea.KeyPressMsg(tea.Key{Code: tea.KeyHome}))
	failIf(t, p.dialog.scroll != 0, "cannot return to preview start")
	p.dialog = nil
	p.wallForm = false
