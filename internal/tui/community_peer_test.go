...
func TestCommunityPeerPartialResponsesAndFencing(t *testing.T) {
	...
	failIf(t, m.community.peer.profile.Username != "Alice" || m.community.peer.profile.State != "offline", "offline resource not applied", m.community.peer)
	...
	failIf(t, m.community.peer.profile.Description != p.Description, "old request replaced profile")
	...
	failIf(t, m.community.peer.profile.Description != "", "reset accepted old profile")
}
func TestCommunityPeerPictureExplicitSaveAndNoOverwrite(t *testing.T) {
	...
	must(t, png.Encode(&data, image.NewNRGBA(image.Rect(0, 0, 2, 2))))
	...
	m.key(chatPress("P"))
	m.key(chatPress("q?"))
	failIf(t, !strings.HasSuffix(m.community.peer.path, "q?") || m.help, "path input invoked global actions")
	...
	for _, size := range [][2]int{{120, 40}, {80, 24}, {40, 16}, {20, 6}} {
		...
		failIf(t, lipgloss.Width(screen) > m.width || lipgloss.Height(screen) > m.height, "picture form overflow", size)
	}
	...
	failIf(t, err != nil || !bytes.Equal(saved, picture), "explicit save", err, m.community.peer.err)
	info, err := os.Stat(path)
	failIf(t, err != nil || info.Mode().Perm() != 0600, "picture permissions", err)
	m.key(chatPress("P"))
	m.community.peer.path = path
	press(tea.KeyEnter)
	press(tea.KeyRight)
	press(tea.KeyEnter)
	failIf(t, !strings.Contains(m.community.peer.err, "exist"), "existing picture overwritten", m.community.peer.err)
	retained, err := os.ReadFile(path)
	failIf(t, err != nil || !bytes.Equal(retained, picture), "existing picture changed", err)
}
...
func TestCommunityPeerAndDiscoveryCacheEviction(t *testing.T) {
	...
	failIf(t, m.community.peer.profile.State != "idle" || m.community.peer.profile.Description != "" || m.community.peer.interests.State != "idle", "evicted resources still appear ready")
	...
	failIf(t, d.state != "idle" || d.generation != 0, "evicted discovery still appears ready", d)
}
...
func TestCommunityPeerKeysRespectInspectorEditor(t *testing.T) {
	...
	failIf(t, m.community.input != "npPr" || m.community.peer.form || m.community.peer.loading, "peer actions captured inspector editor", m.community.input, m.community.peer)
}
...
func TestCommunityPeerRenderedResourceDimensions(t *testing.T) {
	...
	for _, size := range [][2]int{{120, 40}, {110, 40}, {80, 24}, {40, 16}, {20, 6}} {
		m.width, m.height = size[0], size[1]
		screen := m.View().Content
		failIfFmt(t, lipgloss.Width(screen) > m.width || lipgloss.Height(screen) > m.height, "profile overflow %v: %dx%d\n%s", size, lipgloss.Width(screen), lipgloss.Height(screen), screen)
	}
}
...
func TestCommunityPeerVisibleColumnsAndDiscoverSelection(t *testing.T) {
	...
	failIf(t, cmd == nil, "visible inspector never loads at 110 columns")
	drainChat(t, &m, cmd)
	m.width = 109
	failIf(t, m.loadCommunityPeer(false) != nil, "hidden profile was fetched")
